...
main(int argc, char *argv[])
{
    ...
    printf(1, "\n=========== TEST 3: get the dynamically mapped shared memory page (DMSMP) address ===========\n");
    page_addr = get_shared_page_addr(1);
    printf(1, "0x%x ...", page_addr);
    page_addr == (char *)0x4000 ? printf(1, "***PASSED***\n") : printf(1, "***FAILED***\n");
    

    printf(1, "\n=========== TEST 4: test DMSMP functionality - writing shared page before forking ===========\n");    
    if (page_addr == (char *)0)
    {
    ...
    printf(1, "Parent: free frame count = %d\n", n1);

    printf(1, "Parent: writing content to the DMSMP ...\n");
    memset(page_addr, 0, PGSIZE);
    strcpy(page_addr, test_str);

    n2 = get_free_frame_cnt();
    printf(1, "Parent: free frame count = %d ... %s\n", n2, n1 - n2 == 1 ? "***PASSED***" : "***FAILED***");
    ...
    {
	    printf(1, "Child: reading from the SMSP and compare ...");
